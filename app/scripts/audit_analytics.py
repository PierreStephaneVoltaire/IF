...
def build_audit(data: dict, weeks: int) -> str:
    ...
    w("  rpe_stress     = clamp((avg_session_rpe - 6.0) / 4.0, 0, 1)")
    w("                   RPE<=6 -> 0.0 | RPE 8 -> 0.5 | RPE 10 -> 1.0")
    ...
    w("  Formula per set: INOL = (reps / (100 * max(0.01, 1 - I))) * sets")
    w("  where I = kg / estimated_max (relative intensity)")
    w("  Flags: <2.0 = low_stimulus | >4.0 = overreaching_risk")
    w("─" * 60)
    inol = data.get('inol')
    if inol:
    ...
    w()
    w("─" * 60)
    w("SECTION: ACWR (Acute:Chronic Workload Ratio)")
    w("  Formula per dimension: ACWR = acute_mean / chronic_mean")
    w("  acute = last 1 week, chronic = previous 4 weeks (ALL weeks, deloads included)")
    w("  Requires >= 5 completed weeks; returns insufficient_data otherwise.")
    w("  Composite = weighted: axial=0.30, neural=0.30, peripheral=0.25, systemic=0.15")
    w("  Zones: <0.8 undertraining | 0.8-1.3 optimal | 1.3-1.5 caution | >1.5 danger")
    w("─" * 60)
    acwr = data.get('acwr') or {}
    if acwr.get('status') == 'insufficient_data':
        w(f"  INSUFFICIENT DATA: {acwr.get('reason')}")
    else:
        w(f"  Composite: {acwr.get('composite')} ({acwr.get('composite_zone')})")
        for dim, info in acwr.get('dimensions', {}).items():
            w(f"    {dim}: value={info.get('value')} zone={info.get('zone')}")
    ...
    w("    F_axial     = profile.axial * weight * reps")
    w("    F_neural    = profile.neural * reps * φ(I)")
    w("    F_peripheral = profile.peripheral * weight * reps")
    w("    F_systemic  = profile.systemic * weight * reps")
    w("    where φ(I) = ((max(0, I - 0.60)) / 0.40)²")
    ...
    w("  Formula: R = (1 - (0.30*F + 0.25*D + 0.20*S + 0.15*M + 0.10*(1-C))) * 100")
    w("  F = fatigue_norm (0-1) from fatigue_index(14d)")
    w("  D = rpe_drift = |avg_session_rpe - phase_target_mid| / 2, clamped [0,1]")
    w("  S = bw_stability = bodyweight_CV / 0.03, clamped [0,1]")
    w("  M = miss_rate = failed_sets / total_sets (14d)")
    w("  C = compliance_pct from session_compliance(2w)")
    ...
    w(f"    bw_stability: {rc.get('bw_stability', 0)}")
    w(f"    miss_rate: {rc.get('miss_rate', 0)}")
    w(f"    compliance_pct: {rc.get('compliance_pct', 0)}")
